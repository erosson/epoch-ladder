module Pages.Leaderboard exposing (Model, Msg(..), init, subscriptions, toSession, update, view)
...
init code session0 =
    let
        ( session, cmd ) =
            session0 |> Session.fetchLeaderboard code
    in
...
view model =
    [ h1 [] [ text "Last Epoch ladder popularity: ", text model.code ]
